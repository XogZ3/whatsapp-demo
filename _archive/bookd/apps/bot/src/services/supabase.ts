...
export class SupabaseClient {
  ...
  async getStylists(salonId: string, serviceId?: string) {
    ...
    return this.query<Array<Record<string, unknown>>>("wa_stylists", params);
  }
  ...
  async createBooking(data: {
    ...
    const rows = await this.query<Array<Record<string, unknown>>>("wa_bookings", "", {
      method: "POST",
  ...
  async updateBooking(bookingId: string, data: Record<string, unknown>) {
    await this.query("wa_bookings", `id=eq.${bookingId}`, {
      method: "PATCH",
      body: JSON.stringify(data),
    });
  }
  ...
  async getPendingReminders48h() {
    ...
    return this.query<Array<Record<string, unknown>>>(
      "wa_bookings",
      `status=eq.confirmed&reminder_48h_sent=eq.false&starts_at=gte.${h47.toISOString()}&starts_at=lte.${h48.toISOString()}&select=*,wa_clients(*),wa_services(*),wa_salons(*)`
    );
  }
  ...
  async getPendingReminders2h() {
    ...
    return this.query<Array<Record<string, unknown>>>(
      "wa_bookings",
      `status=eq.confirmed&reminder_2h_sent=eq.false&starts_at=gte.${h1.toISOString()}&starts_at=lte.${h2.toISOString()}&select=*,wa_clients(*),wa_services(*),wa_salons(*)`
    );
  }
  ...
  async logMessage(data: {
    ...
    await this.query("wa_message_log", "on_conflict=whatsapp_message_id", {
      method: "POST",
      headers: {
        ...this.headers,
