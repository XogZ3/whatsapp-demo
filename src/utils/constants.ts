...
export function generateSamplePrompts(loraFilename: string) {
  ...
  const samplePrompts = [
    `A bohemian-style ${modelName} travel blogger with sun-kissed skin and messy beach waves, sitting on a tropical beach at sunset. ${modelName} is wearing crisp white clothes and holding up a weathered postcard with “FotoLabs AI” scrawled on it. Golden hour lighting bathes the scene in warm tones.`,
